...
public class FormatValidate {
	...
	/**
	 * 验证用户名注册是否合法-----------由数字、26个英文字母、@、.、-或者下划线组成的字符串
	 * 
	 * @param userName
	 * @return
	 */
...
//		String str = "^\\w+$";
	...
	/**
	 * 验证昵称注册是否合法-----------由数字、26个英文字母、汉字或者_.*() 组成的字符串
	 * 
	 * @param userName
	 * @return
	 */
	public static boolean isRegDisplayName(String displayName) {
		String str = "([\\w*\\.\\(\\)]|[\u4E00-\u9FA5])+";
		boolean flag = true;
		if (displayName != null) {
			Pattern p = Pattern.compile(str);
			Matcher match = p.matcher(displayName);
			flag = match.matches();
		}
		return flag;
	}
	 ...
	 * @return true 通过 false 不通过
	 */
	public static boolean isVaildIP(String ipStr) {
		String str = "((2[0-4]\\d|25[0-5]|[01]?\\d\\d?)\\.){3}(2[0-4]\\|25[0-5]|[01]?\\d\\d?)";
		boolean flag = false;
		if (ipStr != null) {
			Pattern p = Pattern.compile(str);
			Matcher match = p.matcher(ipStr);
			flag = match.matches();
		}
		return flag;
